...
public final class GoodsPage extends ScannerChoice {
	...
	public static void addGoodsPage() {
		...
		System.out.println("\n請輸入添加商品-数量");
		int gnum = ScannerNum();
		Goods goods = new Goods(0, gname, gprice, gnum);
	...
	public static void upateGoodsPage() {
		...
		int gid = QueryPrint.quert("upateGoodsPage");// return the goods gid
		System.out.println("\n--------请选择您要更改的内容\n");
	...
	public static void deleteGoodsPage() {
		...
		// 调用查找商品函数，显示将要删除的商品
		int gid = QueryPrint.quert("deleteGoodsPage");// return the goods gid
		...
		do {
			...
			if ("y".equals(choice) || "Y".equals(choice)) {
				// 进行刪除-数据库操作
				boolean boolDeleteGoods = new GoodsDao().deleteGoods(gid);// 调用刪除功能
				if (boolDeleteGoods) {
				...
				case 3:
					...
					if (goodsList == null || goodsList.size() <= 0) {
						...
						do {
							...
							String choiceNet = ScannerInfoString();
							if ("0".equals(choiceNet)) {
								MainPage.MaintenancePage();
							}
							System.err.println("输入有误！");
						} while (true);
