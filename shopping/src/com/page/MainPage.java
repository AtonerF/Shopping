...
//					mianPage();
	 ...
	 * 购物结算界面
	 */
	private static void shoppingSettlementPage(int sId) {
		// TODO Auto-generated method stub
		System.out.println("\n\t*******购物结算*******\n");
		do {
			...
			if ("0".equals(choNext)) {
				...
				// 当商品件数有且只有一件时返回商品gid号，商品已售空时返回 -1.
				// >1件时返回-2 . 查无此商品时返回-3
				switch (gid) {
				case -3:
					// 无此商品,重新循环
							...
							if (choicegoodsNum > gNum) {
								...
								do {
									...
									if ("y".equals(choShopping) || "Y".equals(choShopping)) {
										...
										System.out.println("\n实际缴费金额");
										do {
											double amoount = ScannerInfo();
											double balance = Arith.sub(amoount, allPrice);// 用户交钱与购买物品总价间的差额
											if (balance < 0) {
												...
												/*
												 * 这里是购物结算操作数据库！！！！！！---------------------- 1.更改goods表数量 2.增加sales表数量
												 * 原商品数量gNum。 结算员Id salesManSid
												 */

												// 对sales表操作
												Gsales gSales = new Gsales(goods.getGid(), sId, choicegoodsNum);
												...
												shoppingSettlementPage(sId);// 最后跳转到到购物结算页面
												// 结束购物结算操作数据库！！！！！！-----------------------------------
											}
										} while (true);
